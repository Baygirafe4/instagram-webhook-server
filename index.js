...
(Последняя запись — 19:00. Раньше 10:00 и позже 19:00 записи не принимаем.)
...
3a. КРИТИЧЕСКИ ВАЖНО про занятость: Список "ЗАНЯТЫЕ СЛОТЫ" выше записан в 24-часовом формате (например 15:00, 17:00). Прежде чем сказать "занято", СНАЧАЛА переведи время клиента в 24-часовой формат: 6 p.m. = 18:00, 7 p.m. = 19:00, 9 a.m. = 9:00, 12 p.m. = 12:00, 1 p.m. = 13:00. ЗАТЕМ проверь есть ли ЭТО ТОЧНОЕ время в списке занятых. Считай время занятым ТОЛЬКО если оно БУКВАЛЬНО есть в списке. Если его там НЕТ — время СВОБОДНО, принимай запись. НИКОГДА не выдумывай занятость. Если список занятых пуст — всё свободно. Пример: список занятых [15:00, 17:00], клиент просит 6 p.m. → это 18:00, в списке его НЕТ → СВОБОДНО, записывай.
...
22. ЧАСЫ РАБОТЫ: Барбершоп работает ежедневно с 10:00 до 19:00. Если клиент спрашивает во сколько работаете / до скольки / когда открыты — ответь: работаем каждый день с 10:00 до 19:00. Если клиент просит записаться на время ВНЕ этих часов (раньше 10:00 или позже 19:00) — вежливо объясни что работаем с 10:00 до 19:00 и предложи выбрать время в этом промежутке. Не принимай запись на нерабочее время.
`;
}
...
// ─── Главная логика ───────────────────────────────────────────────────────────
// Извлекает и нормализует время из текста в формат "H:MM"
// Понимает: 14:00, 14.00, 2 PM, 2:00 PM, 2pm, 6 p.m., 6 p.m, 9 a.m.
function parseTimeFromText(str) {
  if (!str) return null;
  // Нормализуем "p.m." / "a.m." → "pm" / "am", чтобы точки не мешали
  const s = str.replace(/([ap])\s*\.\s*m\s*\.?/gi, '$1m');
  // Час + минуты (опц.) + am/pm
  const full = s.match(/(\d{1,2})(?::|\.)?(\d{2})?\s*(pm|am|рм|ам)\b/i);
  if (full) {
  ...
  // Обычное время с двоеточием / точкой
...
const TRANSLATIONS = {
  ...
  outsideHours: {
    'русский': 'К сожалению, {t} — это вне рабочих часов 😔 Мы работаем ежедневно с 10:00 до 19:00. Выберите пожалуйста время в этом промежутке!',
    'польский': 'Niestety {t} to poza godzinami pracy 😔 Pracujemy codziennie od 10:00 do 19:00. Wybierz proszę godzinę w tym przedziale!',
    'английский': 'Unfortunately {t} is outside our working hours 😔 We\'re open daily from 10:00 to 19:00. Please choose a time within these hours!'
  },
  accidental: {
...
// Жёсткая проверка рабочих часов (10:00–19:00) — даже если ИИ ошибётся
if (userTime && !conv.awaitingTimeConfirm) {
  const [uh, um] = userTime.split(':').map(Number);
  const minutes = uh * 60 + (um || 0);
  const OPEN = 10 * 60;   // 10:00
  const CLOSE = 19 * 60;  // 19:00 — последняя запись
  if (minutes < OPEN || minutes > CLOSE) {
    await sendInstagramMessage(senderId, t('outsideHours', conv, userTime), business.accessToken);
    return;
  }
}
...
if (userTime && !conv.awaitingTimeConfirm) {
  ...
  if (checkDate) {
    ...
    if (taken) {
      ...
      // Слот СВОБОДЕН по данным базы — говорим об этом ИИ явно, чтобы он не выдумал "занято"
      conv.messages.push({
        role: "user",
        content: `[КОНТЕКСТ ДЛЯ ТЕБЯ, не показывай клиенту] Система проверила базу: ${checkDate} в ${userTime} — СВОБОДНО. Принимай эту запись, НЕ говори что занято.`
      });
    }
  }
}
